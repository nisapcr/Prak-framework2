...
export default function Header() {
    ...
    return (
        /* flex-none ditambahkan agar header tidak mengambil ruang lebih/mendorong konten */
        <div className="flex-none flex justify-between items-center mb-8 bg-white/50 backdrop-blur-md p-4 rounded-2xl border border-slate-100 shadow-sm relative z-40">
            ...
            {/* MODAL SEARCH (NO-SCROLL VERSION) */}
            {openSearch && (
                /* overflow-hidden pada backdrop memastikan tidak ada scroll saat modal aktif */
                <div
                    className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-start justify-center z-50 pt-20 px-4 overflow-hidden"
                    onClick={() => setOpenSearch(false)}
                >
                    <div
                        className="bg-white rounded-[2rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 flex flex-col max-h-[80vh]"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Search Input */}
                        <div className="p-6 border-b border-slate-100 flex items-center gap-4 flex-none">
                            <FaSearch className="text-emerald-500 text-xl" />
                            <input
                                autoFocus
                                type="text"
                                className="flex-1 text-lg font-medium focus:outline-none placeholder:text-slate-300 bg-transparent"
                                placeholder="What are you looking for?"
                                value={keyword}
                                onChange={(e) => setKeyword(e.target.value)}
                            />
                            <button onClick={() => setOpenSearch(false)} className="p-2 hover:bg-slate-100 rounded-full">
                                <FaTimes className="text-slate-400" />
                            </button>
                        </div>

                        {/* Scrollable Results (hanya bagian ini yang boleh scroll jika item banyak, bukan seluruh halaman) */}
                        ...
                        {/* Footer Modal */}
                        <div className="p-4 px-8 bg-white border-t border-slate-100 flex justify-between items-center flex-none">
                            <p className="text-[10px] text-slate-400">Tip: Press ESC to close</p>
                            <button
